...
/basestation   Raspberry Pi 5: sensor drivers, local API, kiosk UI, offload   <- MOVING
/docs          architecture + to-do docs

> **MOVING.** The basestation stack is moving to `isensystech/mobile-lab-station`. See
> [docs/BaseStation-Arch.md](docs/BaseStation-Arch.md) for the succession notice.
...
3. Base station (MOVING to `isensystech/mobile-lab-station`) — Pi AP + Timescale -> one
   Modbus driver -> MQTT -> offload -> kiosk UI. Do not start new work here.
